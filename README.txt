...
micropositioning devices. We'll cover the functionality of the scanners along with any known bugs and prefered options. If you have any questions 
please feel free to reach out to either Hayden Binger or Devon Uram at binge063@umn.edu or uram0283@umn.edu.

Upon running the code the user will be prompted with a new subVI window. In that window you will need to accuratly put in the com-port that the 
...
- Heatmap Z value bar not updating properly durring and after a scan. 
- Min value in angled scans.

List of features and descrition of features:
=====================================================================================================================================================
...
Encoder Position vs. Calculated Postion:
At the top of the screen there are two seperate indicators, one for enc position and one for calc position. "enc position" is the position as 
...
Move Commands: 
There are two seperate types of move commands described bellow:
Absolute movement/ move to:
This will move the motors to the specifed position at the speed as set by the motion parameters. This is most useful for covering large distances 
...
The general syntax is: (controllor number)(three letter code for the command)(optional number if one is required)
...
Loop type:
There are three loop types availible. To switch between them type in the number corresponding to the desired loop type and click update loop type. 
...
Limits:
Soft limits are limts that stop the motors from moving past the specified point. If a motor would recieve a movement command that would move it
...
or if you accidentily set the soft limit spot before your current position, you can still use the absolute movement to get back to the operable range
...
Scanning patterns:
The bulk of the use of the code comes from the scanning patterns. There are many controls described bellow:
Port number: The aqusition of data is done through a DAQ task. In order to build the task correctly you will need to input both the port the DAQ is 
...
of each scan the code moves back to the origin for conveince. 
Finally there are four unique type of scanning patterns: 
Center Serpant Scan: Starts a serpant scan centered at the scan origin. A serpant scan is charachterized by the negative moving backstrokes in the 
...
Cetner Typewritter Scan: Starts a typewritter scan centered at the scan origin. A typewritter scan is charachterized by always moving in the positive
...
continue the scan. 
Start Point Serpant Scan: Starts a serpant scan where the starting point is the bottom left measurement.
Start Point Typewritter Scan: Starts a typewritter scan where the starting point is the bottom left measurement.
...
- The prefered scanning pattern is typewritter scanning as the movements add limited extra time while drasticly improving the quality of the scans. 
...
is updated to correspond to the actual position and scale matching the scanning parameters. The measured ad averaged voltage will be displayed for 
...
singular loop are displayed at each step. At the end of each fast axis line the maximum, the minimum, and the position at which the maximum occurs 
